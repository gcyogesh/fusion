    ...
    const TestimonialCard: React.FC<TestimonialCardProps> = ({ message, name, position, profileImage, rating }) => {
      return (
        <div className="flex flex-col min-w-[320px] max-w-[320px] md:max-w-[365px] h-[260px] md:h-[235px]   bg-[#FCE1AC] rounded-xl p-4  snap-center flex-shrink-0 shadow-md">
        
          {/* Testimonial Icon */}
      <FaQuoteRight className="absolute top-50 hidden md:top-60 lg:top-60 right-40 lg:right-70 text-blue text-xl  opacity-40" />
          ...
          <TextDescription text={message} className="w-full h-[150px] text-sm text-bold text-black mt-2" />
    ...
    const TestimonialCarousel: React.FC<TestimonialCarouselProps> = ({ testimoinaldata }) => {
      return (
        <div className="relative px-4 py-10">
          ...
          <div className="flex  overflow-x-auto snap-x snap-mandatory gap-4 px-1 py-6 scrollbar-hide sm:justify-center sm:flex-wrap bg-[linear-gradient(90deg,_#FEF9EE_0.88%,_#1C9ADB_32.63%,_#0F7BBA_70.26%,_#FEF9EE_100%)]  sm:bg-[linear-gradient(90deg,_#FEF9EE_0.88%,_#0F7BBA_50%,_#FEF9EE_100%)]  md:bg-[linear-gradient(90deg,_#FEF9EE,_#1C9ADB,_#FEF9EE)]"
            style={{
