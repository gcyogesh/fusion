...
export default function Page({ params }: { params: Promise<{ type: string; slug: string }> }) {
  ...
  const totalPages = Math.ceil(relatedPackages.length / packagesPerPage);
  const paginatedPackages = relatedPackages.slice(
    (currentPage - 1) * packagesPerPage,
    currentPage * packagesPerPage
  );

  return (
    <div className="w-full min-h-screen bg-gradient-to-br from-[#fef9ee] via-[#f7e6d0] to-[#f8e0c2] text-gray-900 py-10 px-4">
      <Alert
        show={alert.show}
        type={alert.type}
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          {!showForm && (
            <Button
              ...
              className="!bg-gradient-to-r !from-primary !to-orange-400 !text-white !px-6 !py-2 !rounded-lg !shadow-lg hover:!bg-primary/90 transition !ring-2 !ring-primary/30"
              onClick={handleAdd}
            />
          )}
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center h-64 mb-8">
            <svg className="animate-spin h-14 w-14 text-primary mb-4 drop-shadow-lg" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path>
            </svg>
            <span className="text-primary text-lg font-bold tracking-wide animate-pulse">Loading packages...</span>
          </div>
        ) : (
          ...
          relatedPackages.length > 0 ? (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
                {paginatedPackages.map((pkg) => (
                  <div key={pkg._id} className="transition-transform hover:scale-[1.03] hover:shadow-2xl duration-200">
                    <PackageCard
                      pkg={pkg}
            ...
            <div className="flex flex-col items-center justify-center h-64 mb-8 bg-gradient-to-br from-white via-orange-50 to-primary/10 rounded-xl shadow-inner border border-gray-100">
              ...
              <span className="text-xl font-semibold text-primary">No packages found</span>
        ...
        {/* Error */}
        {errorMessage && (
          <div className="text-red-600 text-sm mb-4 font-semibold">{errorMessage}</div>
        )}
      </div>
    </div>
  );
